...
from .layers import subpel_conv3x3, subpel_conv1x1, DepthConvBlock, \
    ResidualBlockWithStride, ResidualBlockUpsample,Base_Extractor, Detail_extractor, Fuse_encoder, Fuse_decoder
...
class DMC(CompressionModel):
    def __init__(self, anchor_num=4, ec_thread=False, stream_part=1, inplace=False):
        ...
        self.decoder_side_feature_adaptor = nn.Conv2d(3, g_ch_1x, 3, padding=1)
        self.decoder_side_feature_weighter = nn.Conv2d(g_ch_1x*2, g_ch_1x, 3, padding=1)

        self.fuse_encoder = Fuse_encoder(in_ch=g_ch_1x, out_ch=64)
        self.base_fuse = Base_Extractor(N=64)
        self.detail_fuse = Detail_extractor(N=64)
        self.fuse_docoder = Fuse_decoder(dim=64, out_channels=g_ch_1x)
    ...
    def decoder_side_memc(self, refer_frame, predicted_frame):
        ...
        decoder_side_mcfeature = self.decoder_side_feature_adaptor(decoder_side_mcframe)
        return decoder_side_mcframe, decoder_side_mcfeature
    
    def decoder_context_refine(self, context_flow, context_deflow):
        context_flow_base, context_flow_deta, context_deflow_base, context_deflow_deta = self.fuse_encoder(context_flow, context_deflow)
        decom_loss = self.decomple_loss(context_flow_base, context_deflow_base, context_flow_deta, context_deflow_deta)
        fuse_base = self.base_fuse(context_flow_base+context_deflow_base)
        fuse_detail = self.detail_fuse(context_flow_deta+context_deflow_deta)
        decoder_memc_feature = self.fuse_docoder(context_flow, fuse_base, fuse_detail)

        return decoder_memc_feature, decom_loss

    def motion_compensation(self, dpb, mv, index):
        warpframe = flow_warp(dpb["ref_frame"], mv)
        decoder_side_mcframe, decoder_side_mcfeature = self.decoder_side_memc(refer_frame=dpb["ref_frame"], predicted_frame=warpframe)
        ...
        context1, decom_loss = self.decoder_context_refine(context1, decoder_side_mcfeature)
        return context1, context2, context3, warpframe, decoder_side_mcframe, decom_loss
    ...
    def forward_one_frame(self, x, dpb, q_in_ckpt=False, q_index=None, frame_idx=0):
        ...
        context1, context2, context3, warp_frame, decoder_side_mcframe, decom_loss = self.motion_compensation(dpb, mv_hat, frame_idx)
